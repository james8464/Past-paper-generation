...
class TerminalProgressReporter:
    ...
    def _render(self, snapshot: ProgressSnapshot, final: bool = False) -> str:
        ...
        lines = [
            "AQA A-Level Computer Science Paper 2 Generator".ljust(width),
            f"{spinner}  FPS {self.fps}   Elapsed {_format_elapsed(snapshot.elapsed_seconds)}".ljust(width),
            f"Seed: {snapshot.seed or '-'}".ljust(width),
            f"Steps: {snapshot.completed_steps}/{snapshot.total_steps}  {_progress_bar(snapshot.completed_steps, snapshot.total_steps, 28)}".ljust(width),
            f"Stage: {snapshot.current_stage}".ljust(width),
            f"Question: {_question_label(snapshot)}  {_progress_bar(snapshot.questions_done, snapshot.question_total, 28)}".ljust(width),
            "-" * width,
            "Recent".ljust(width),
        ]
