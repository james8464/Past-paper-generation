...
GENERATOR_ROOT = ROOT / "Resources" / "business" / "aqa" / "generator"
...
def build_reference_profile(corpus_root: Path = CORPUS_ROOT) -> dict[str, Any]:
    ...
    papers: dict[str, Any] = {}
    for paper in ("1", "2", "3"):
        paths = sorted(question_root.glob(f"AQA-7132{paper}-QP-*.PDF"))
        if not paths:
            raise ValueError(f"no official AQA Business Paper {paper} references found")
        current = question_root / f"AQA-7132{paper}-QP-JUN25.PDF"
        if current not in paths:
            raise ValueError(f"current AQA Business Paper {paper} reference is missing")
        source_paths.extend(paths)
        texts = [_pdf_text(path) for path in paths]
        sequences = [
            [int(value) for value in MARK_PATTERN.findall(text)] for text in texts
        ]
        current_text = _pdf_text(current)
        papers[paper] = {
            "documents": len(paths),
            "current_printed_mark_sequence": [
                int(value) for value in MARK_PATTERN.findall(current_text)
            ],
    ...
    inserts = sorted((corpus_root / "inserts").glob("AQA-71323-INS-*.PDF"))
    if not inserts:
        raise ValueError("no official AQA Business Paper 3 inserts found")
    source_paths.extend(inserts)
    insert_texts = [_pdf_text(path) for path in inserts]
    papers["3"]["source_insert"] = {
        "documents": len(inserts),
        "page_count": _band([_page_count(path) for path in inserts]),
        "document_word_count": _band(
            [len(WORD_PATTERN.findall(text)) for text in insert_texts]
        ),
    }
    return {
...
def build_generated_profile(
    ...
    for paper_number in ("1", "2", "3"):
        ...
        with tempfile.TemporaryDirectory(
            prefix=f"examforge-aqa-business-calibration-{paper_number}-"
        ) as folder:
            ...
            insert: dict[str, int] | None = None
            if paper_number == "3":
                insert_path = Path(folder) / "insert.pdf"
                render_source_booklet(sample, insert_path)
                insert = {
                    "page_count": _page_count(insert_path),
                    "document_word_count": len(
                        WORD_PATTERN.findall(_pdf_text(insert_path))
                    ),
                }
        ...
        if paper_number == "1":
            ...
            quantitative = sum(question.number in {"05", "10", "15"} for question in mcqs)
            checks["mcq_demand_mix"] = quantitative == 3 and len(mcqs) == 15
        ...
        if paper_number == "3":
            ...
            checks["source_insert_word_count"] = (
                insert_band["document_word_count"]["minimum"]
                <= insert["document_word_count"]
                <= insert_band["document_word_count"]["maximum"]
            )
            profile["source_insert"] = insert
        papers[paper_number] = profile
...
def build_report(corpus_root: Path = CORPUS_ROOT) -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "qualification": "AQA A-level Business 7132",
        "purpose": (
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Calibrate AQA Business generated-paper structure."
    )
    ...
    if args.check:
        if not OUTPUT.exists() or OUTPUT.read_text(encoding="utf-8") != rendered:
            raise SystemExit(
                f"{OUTPUT} is stale; run tools/aqa_business_calibration.py --write"
            )
        return 0
    print(rendered, end="")
    return 0
