...
def handle_benchmark(args: argparse.Namespace) -> int:
    ...
    while True:
        ...
        remaining_tick = 1.0 - ((time.perf_counter() - started) % 1.0)
...
def emit_final_metrics(samples: list[dict[str, float | None]]) -> None:
    ...
    cpu_score = clamp(cpu / 650)
    load_score = clamp(1.0 - (cpu_load / 100) * 0.7)
    memory_score = clamp(free_memory / 8)
    disk_score = (clamp(disk_write / 400) + clamp(disk_read / 700)) / 2
    network_score = 0.72 if latency is None else clamp(1.0 - min(latency, 900) / 1000)
    score = (
        cpu_score * 0.34
        + load_score * 0.16
        + memory_score * 0.2
    ...
    emit("benchmark_metric", name="Disk Write", value=disk_write, unit="MB/s", score=clamp(disk_write / 400))
    emit("benchmark_metric", name="Disk Read", value=disk_read, unit="MB/s", score=clamp(disk_read / 700))
    if latency is None:
...
def cpu_probe(payload: bytes) -> float:
    started = time.perf_counter()
    processed = 0
    while time.perf_counter() - started < 0.18:
        hashlib.sha256(payload).digest()
        processed += len(payload)
    elapsed = max(0.001, time.perf_counter() - started)
    return processed / elapsed / 1_000_000
...
def available_memory_gb() -> float:
    if platform.system() == "Darwin":
        try:
            ...
            return free_pages * page_size / 1_073_741_824
        except Exception:
            pass
...
def total_memory_gb() -> float:
    if platform.system() == "Darwin":
        try:
            ...
            return int(raw.strip()) / 1_073_741_824
        except Exception:
            pass
